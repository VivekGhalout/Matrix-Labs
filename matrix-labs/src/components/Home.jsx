import { Box, Button, Flex, Input, InputGroup, InputRightElement, Drawer, DrawerBody, DrawerHeader, DrawerOverlay, DrawerContent, DrawerCloseButton, useDisclosure, Text, } from '@chakra-ui/react'
...
function Home() {
    ...
    const [address, setAddress] = useState("token");
    ...
    const debouncedFetchData = debounce((query) => {
        const apiUrl = `https://api.dexscreener.com/latest/dex/search/?q=${query}`;


        fetch(apiUrl)
            .then((response) => response.json())
            .then((data) => {
                const sortedData = data.pairs.sort((a, b) => a.priceUsd - b.priceUsd);
    ...
    return (
        <Box backgroundImage={bg_image} backgroundAttachment={"fixed"}>
            <Flex>
                <Box w={"16%"} h={"90vh"} borderRadius={"0px 32px 0px 0px"} bg={"#292929"} display={{ base: "none", md: "none", lg: "block" }}>
                    <Sidebar address={address} setAddress={setAddress} setSearchResults={setSearchResults} />
                </Box>
                <Box w={{ base: "100%", md: "100%", lg: "84%" }} p={"30px 40px"}>
                    <Box>
                        <Flex px={{ base: "0px", md: "0px", lg: "30px" }} justifyContent={"space-between"} alignItems={"center"}>
                            <Box display={{ base: "block", md: "block", lg: "none" }}>
                                ...
                                <Drawer placement={"left"} onClose={onClose} isOpen={isOpen}>
                                    <DrawerOverlay />
                                    <DrawerContent>
                                        <DrawerHeader bg={"#292929"}></DrawerHeader>
                                        <Flex gap={"20px"} alignItems={"center"} p={"20px 34px"} bg={"#292929"}>
                                            ...
                                            <Text fontFamily={"Pacifico"} fontWeight={"400"} fontSize={"32px"} color={"#ffffff"}>NFTify</Text>
                                        </Flex>
                                        <DrawerBody bg={"#292929"}>
                                            <Flex gap={"20px"} alignItems={"center"} h={"65px"} bg={address === "token" ? "#F30050" : "none"} px={"34px"} onClick={() => { setAddress("token") }} cursor={"pointer"}>
                                                ...
                                                <Text fontWeight={"600"} fontSize={"20px"} lineHeight={"28px"} color={"#F7F9F9"}>Token Address</Text>
                                            </Flex>

                                            <Flex gap={"20px"} alignItems={"center"} h={"65px"} bg={address === "pair" ? "#F30050" : "none"} px={"34px"} onClick={() => { setAddress("pair") }} cursor={"pointer"}>
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                <Text fontWeight={"400"} fontSize={"20px"} lineHeight={"28px"} color={"#F7F9F9"}>Pair Address</Text>
                                            </Flex>
                                        </DrawerBody>
                                    </DrawerContent>
                                </Drawer>
                            ...
                            <Box display={{ base: "none", md: "none", lg: "block" }}>
                                <InputGroup w={'435px'} h={"50px"} borderRadius={"20px"} border={"1px solid #ffffff"}>
                                    <Input
                                        pr='4.5rem'
                                        type='text'
                                        placeholder='Search'
                                    ...
                                    <InputRightElement width='4.5rem'>
                                        <BiSearch color='#ffffff' size={"30px"} />
                                    </InputRightElement>
                                </InputGroup>
                            </Box>

                            <Button colorScheme='none' w={"156px"} h={"52px"} bgGradient={"linear(to-r, #7C0F35, #581266)"} borderRadius={"20px"} fontSize={"16px"} fontWeight={"600"}>
                                Connect
                            </Button>
                        </Flex>
                        <Box display={{ base: "block", md: "block", lg: "none" }} mt={"20px"}>
                            <InputGroup w={'100%'} h={"50px"} borderRadius={"20px"} border={"1px solid #ffffff"}>
                                <Input
                                    pr='4.5rem'
                                    type='text'
                                    placeholder='Search'
                        ...
                        <Box mt={"50px"}>
                            {
                                address === "token" ? <TokenAddress searchResults={searchResults}/> : <PairAddress searchResults={searchResults}/>
                            }
            ...
            <Box w={"100%"} h={"68px"} background={"#F30050"}>
